<div class="z-50 navbar fixed bg-base-200 shadow-xl" id="nav">
  <div class="flex-1">
    <a class="btn btn-ghost normal-case text-2xl" href="/index">
      <img src="/img/logo.avif" onerror="this.src='/img/logo.png'" class="w-10 mr-2">FRHS CS
    </a>
  </div>
  <div class="flex-none hidden md:flex">
    <ul class="menu menu-horizontal px-1 gap-3 mr-3">
      ...
      <!-- Beardify Link (conditionally visible in November) -->
      <li id="beardify-link" style="display: none;">
        <a href="/beardify">Beardify</a>
      </li>
...
<script>
  // JavaScript to show/hide the "Beardify" link based on the current month
  const currentMonth = new Date().getMonth(); // 0 = January, 10 = November, etc.
  const beardifyLink = document.getElementById('beardify-link');

  if (currentMonth === 10) { // November (month index 10)
    beardifyLink.style.display = 'block';
  } else {
    beardifyLink.style.display = 'none';
  }
</script>
